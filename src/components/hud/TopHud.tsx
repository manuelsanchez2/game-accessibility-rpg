...
const TopHud = ({ level }: { level: LevelProps }) => {
  ...
  // TODO: SETTINGS for Zustand later
  // DISPLAY SETTINGS
  const textColor = '#fff'
  const bgColor = '#FF9A03'
  const textSize = isMobile ? '0.5rem' : '0.9rem'

  const iconSize = useMemo(() => {
    const sizeInRem = parseFloat(textSize.replace('rem', ''))
  ...
  return (
    <div className="z-10 absolute top-0 w-full flex items-center justify-between pl-6 pr-12">
      <div className="w-fit relative">
        <TopHudCharacter bgColor={bgColor} />
...
function TopHudCharacter({ bgColor }: { bgColor: string }) {
  const { isMobile } = useWindowWidth()
